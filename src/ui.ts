...
export class GameUI {
  ...
  constructor(private root:HTMLElement,private actions:UIActions){
    ...
    root.addEventListener('click',e=>{
      ...
      const {action,class:cls,upgrade}=button.dataset;
  ...
  private renderOverlay(s:UIState):string{
    ...
    const won=s.phase==='won';return `<div class="modal-shade end-shade"><section class="end-panel ornamental"><div class="end-sigil">${won?'☼':'✧'}</div><div class="eyebrow">${won?'THE NIGHT HAS ENDED':'THE KEEP REMEMBERS YOU'}</div><h2>${won?'长夜终焉':'余烬未灭'}</h2><p>${won?'黎明穿过断壁。你的名字留在古堡深处。':esc(s.notice || '行者倒下了，但古堡的门仍为你敞开。')}</p><div class="end-stats"><div><b>${s.floor}</b><span>抵达层数</span></div><div><b>${s.kills}</b><span>斩灭魔物</span></div><div><b>${Math.floor(s.elapsed/60)}:${String(Math.floor(s.elapsed%60)).padStart(2,'0')}</b><span>征途时长</span></div></div><p class="best-run">最高击破记录 · ${Math.max(s.best,s.kills)}</p><button class="primary" data-action="restart">${won?'开启新的长夜':'再次踏入古堡'} <span>⟶</span></button><button class="text-button" data-action="menu">重选行者</button></section></div>`;
  }
  private renderMap(s:UIState){
    ...
    const labels:Record<string,string>={entrance:'入口',combat:'战斗',elite:'精英',boss:'领主',treasure:'宝库',sanctuary:'圣所'};
    return `<div class="modal-shade"><section class="map-panel ornamental"><div class="map-heading"><div><div class="eyebrow">CARTOGRAPHY OF THE UNKNOWN</div><h2>古堡秘图 <small>第 ${s.floor} 层</small></h2></div><button class="icon-button" data-action="map" aria-label="关闭地图">×</button></div><p>击败两位精英，集齐印记后挑战本层领主。</p><div class="map-pan-hint">← 横向滑动查看完整楼层 →</div><div class="map-scroll"><svg class="castle-map" viewBox="0 0 ${width} ${height}" role="img" aria-label="当前楼层房间路线图">${edges}${s.rooms.map(r=>{const p=pos(r.id);return `<g class="map-room ${r.visited?'visited':''} ${r.id===s.roomId?'current':''} ${r.cleared?'cleared':''}" transform="translate(${p.x},${p.y})"><rect x="-24" y="-17" width="48" height="34" rx="2"/><text class="map-symbol" text-anchor="middle" y="5">${r.kind==='boss'?'♛':r.kind==='elite'?'◆':r.kind==='treasure'?'✧':r.kind==='sanctuary'?'✚':r.kind==='entrance'?'⌂':'·'}</text><text class="map-label" text-anchor="middle" y="34">${labels[r.kind]}${r.id===s.roomId?' · 当前':''}</text></g>`;}).join('')}</svg></div><div class="map-legend"><span><i class="current-key"></i>当前位置</span><span><i></i>已探索</span><span>┄ 能力封印</span><span>◆ 精英　♛ 领主</span></div><div class="map-footer"><span>${s.doubleJump?'✦ 已获得二段跳':'◇ 击败首位精英：二段跳'}<br>${s.breakDash?'✦ 已获得破障冲刺':'◇ 击败第二位精英：破障冲刺'}</span><button class="secondary" data-action="map">继续探索 <span>M</span></button></div></section></div>`;
  }
}
