...
export class GameUI {
  ...
  constructor(private root:HTMLElement,private actions:UIActions){
    ...
    root.addEventListener('input',e=>{const el=e.target as HTMLInputElement;if(el.matches('[data-music-volume]')){this.actions.setMusicVolume(Number(el.value)/100);const label=root.querySelector('[data-music-level]');if(label)label.textContent=el.value+'%';}});
  ...
  update(s:UIState){
    ...
    this.text('mute',s.muted?'♫̸':'♪');this.refs.mute.setAttribute('aria-label',s.muted?'开启声音':'关闭声音');
  ...
  private renderOverlay(s:UIState):string{
    ...
    if(s.phase==='paused')return `<div class="modal-shade"><section class="pause-panel ornamental"><div class="eyebrow">A MOMENT BETWEEN SHADOWS</div><h2>长夜暂歇</h2><p>古堡的低语仍在回荡。</p><button class="primary" data-action="resume">继续征途 <span>ESC</span></button><div class="setting-row"><span>全部声音<small>音乐、战斗与交互声音</small></span><button class="toggle" data-action="mute" aria-pressed="${!s.muted}">${s.muted?'关闭':'开启'}</button></div><div class="setting-row"><span>减少动态效果<small>降低镜头震动与闪烁</small></span><button class="toggle" data-action="motion" aria-pressed="${s.reducedMotion}">${s.reducedMotion?'开启':'关闭'}</button></div><div class="music-setting"><label for="music-volume">背景音乐 <span data-music-level>${Math.round(s.musicVolume*100)}%</span></label><input id="music-volume" data-music-volume type="range" min="0" max="100" step="5" value="${Math.round(s.musicVolume*100)}" aria-label="背景音乐音量"/><small>探索与领主战自动切换</small></div><button class="text-button" data-action="menu">返回古堡入口</button></section></div>`;
