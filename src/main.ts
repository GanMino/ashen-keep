...
function loop(now:number){const dt=Math.min(.1,(now-last)/1000);last=now;frameTime=frameTime*.95+dt*1000*.05;acc+=dt;while(acc>=1/60){game.tick(1/60);acc-=1/60;}renderer.render(game);ui.update(game.getState());frames++;requestAnimationFrame(loop);}requestAnimationFrame(loop);
if(import.meta.env.DEV||new URLSearchParams(location.search).has('test')){
 ...
 window.__THREE_GAME_TEST_HOOKS__={seed:(n:number)=>{game.seed=n;game.rng=()=>.5;},setPausedForScreenshot:(v:boolean)=>{game.frozen=v;},setReducedMotion:(v:boolean)=>{game.reducedMotion=v;},hideDebugUi:()=>{},setState:(state:string)=>{
   if(!['menu','active-play','boss','paused','dead','upgrade'].includes(state))throw new Error('Unknown state '+state);
   game.start(game.seed);game.time=8;
   if(state==='menu'){game.phase='menu';game.preview();}
   else if(state==='boss'){game.elites=2;game.doubleJump=true;game.breakDash=true;game.travel(game.rooms.find(r=>r.kind==='boss')!.id);game.player.x=600;game.cameraX=220;}
   else{game.travel(1);game.player.x=510;game.player.y=440;game.cameraX=180;game.player.attack=.14;game.room.enemies.forEach((e,i)=>{if(i<5){e.x=650+i*48;e.y=e.kind==='bat'?330:440;}});if(state==='paused')game.pause();if(state==='dead'){game.player.hp=0;game.phase='dead';}if(state==='upgrade'){game.xp=game.xpNext;game.checkLevel();}}
