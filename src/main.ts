...
const ui=new GameUI(document.querySelector<HTMLElement>('#ui')!,{
 selectClass:id=>{game.selectedClass=id;game.sound.play('loot');},start:()=>game.beginRun(),pause:()=>game.pause(),resume:()=>game.resume(),restart:()=>game.beginRun(),menu:()=>{game.phase='menu';game.clearInput();game.preview();},toggleMap:()=>game.toggleMap(),toggleMute:()=>game.toggleMute(),toggleMotion:()=>{game.reducedMotion=!game.reducedMotion;},chooseEvent:id=>game.chooseEvent(id),chooseUpgrade:id=>game.selectUpgrade(id),input:(key,down)=>{game.sound.unlock();game.input(key,down);}
});
...
if(import.meta.env.DEV||new URLSearchParams(location.search).has('test')){
 ...
 window.__THREE_GAME_TEST_HOOKS__={seed:(n:number)=>{game.seed=n;game.rng=()=>.5;},setPausedForScreenshot:(v:boolean)=>{game.frozen=v;},setReducedMotion:(v:boolean)=>{game.reducedMotion=v;},hideDebugUi:()=>{},setState:async(state:string)=>{
   ...
   if(!['menu','active-play','boss','elite','boss-phase2','paused','dead','upgrade','story','shop','final-boss','library','crypt','clock','ending'].includes(state))throw new Error('Unknown state '+state);
   game.start(game.seed);game.time=8;
   if(state==='story'){game.beginRun(game.seed);}
