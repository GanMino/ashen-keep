...
export class Game {
 ...
 clearEffects(){this.particles=[];this.floaters=[];this.shots=[];this.pickups=[];this.hitstop=0;this.shake=0;}
 ...
 float(x:number,y:number,text:string,color='#f1d7a1'){this.floaters.push({x,y,text,color,life:.85});}
 hurtEnemy(e:Enemy,damage:number,knock=60){if(e.dead)return;e.hp-=damage;e.flash=.13;e.vx+=Math.sign(e.x-this.player.x)*knock;this.float(e.x,e.y-(e.kind==='boss'?105:50),String(Math.round(damage)));this.burst(e.x,e.y-25,e.kind==='bat'?'#8ebbb7':'#c0bfac',5);this.sound.play('hit');
  if(e.hp<=0){e.dead=true;this.kills++;this.combo++;this.comboTime=3;this.player.hp=Math.min(this.player.maxHp,this.player.hp+this.lifesteal);this.sound.play('kill');this.burst(e.x,e.y-25,'#aee8d2',14);
 ...
 attack(){const p=this.player;p.attack=.2;this.attackCd=this.attackRate;this.attackCount++;this.sound.play('hit');
  if(this.selectedClass==='knight'){
   const range=108*this.reach;for(const e of this.room.enemies)if(!e.dead&&Math.abs(e.y-p.y)<95&&Math.abs(e.x-p.x)<range&&(e.x-p.x)*p.facing>-30)this.hurtEnemy(e,this.damage,125);
  }else{
   const count=(this.selectedClass==='witch'?2:1)+this.extraShots;
   for(let i=0;i<count;i++){const angle=(i-(count-1)/2)*.13;this.shoot(p.x+p.facing*20,p.y-28,p.facing*Math.cos(angle)*(this.selectedClass==='ranger'?700:450),Math.sin(angle)*300,this.damage,false,this.selectedClass==='witch'?'#efb882':'#a3d4bd',this.selectedClass==='witch'?7:3,this.selectedClass==='ranger'?3:2);}
  }
  if(this.thunder&&this.attackCount%4===0){const target=this.room.enemies.find(e=>!e.dead&&Math.abs(e.x-p.x)<550);if(target){this.burst(target.x,target.y-30,'#b8eff0',35);for(const e of this.room.enemies)if(!e.dead&&Math.abs(e.x-target.x)<150)this.hurtEnemy(e,55*this.thunder,60);}}
 }
 skill(){const p=this.player;this.skillCd=this.skillRate;this.sound.play('skill');this.shake=.22;p.attack=.4;p.invuln=Math.max(p.invuln,.4);
  const radius=this.selectedClass==='knight'?235:360;this.burst(p.x,p.y-25,this.selectedClass==='witch'?'#edb274':'#d5f5d6',65);
  for(const e of this.room.enemies)if(!e.dead&&Math.abs(e.x-p.x)<radius&&Math.abs(e.y-p.y)<180)this.hurtEnemy(e,this.damage*3.6,230);
  if(this.selectedClass!=='knight')for(let i=-3;i<=3;i++)this.shoot(p.x,p.y-25,p.facing*(470-Math.abs(i)*20),i*70,this.damage*1.8,false,'#f3d59d',7,5);
  this.float(p.x,p.y-95,this.selectedClass==='knight'?'月刃风暴':this.selectedClass==='witch'?'烬火爆裂':'千羽齐射','#e8d8ab');
 }
 ...
 tick(dt:number){
  ...
  this.time+=dt;this.elapsed+=dt;this.noticeTime-=dt;if(this.noticeTime<=0)this.notice='';this.shake=Math.max(0,this.shake-dt);if(this.hitstop>0){this.hitstop-=dt;return;}
  const p=this.player;this.attackCd-=dt;this.skillCd=Math.max(0,this.skillCd-dt);this.dashCd=Math.max(0,this.dashCd-dt);p.invuln=Math.max(0,p.invuln-dt);p.attack=Math.max(0,p.attack-dt);p.dash=Math.max(0,p.dash-dt);this.comboTime-=dt;if(this.comboTime<=0)this.combo=0;
  ...
  if(this.pressed.has('Space')||this.pressed.has('KeyW')||this.pressed.has('ArrowUp')){if(p.grounded||(this.doubleJump&&p.jumps<2)){p.vy=-610;p.grounded=false;p.jumps++;this.sound.play('jump');this.burst(p.x,p.y,'#8fa7a6',6);}}
  ...
  p.vx=p.dash>0?p.facing*850:axis*this.speed;
  const oldY=p.y;p.x=Math.max(28,Math.min(this.room.width-28,p.x+p.vx*dt));p.vy+=1480*dt;p.y+=p.vy*dt;p.grounded=false;
  if(p.vy>=0){let landing=GROUND;for(const platform of this.room.platforms)if(p.x>platform.x-10&&p.x<platform.x+platform.w+10&&oldY<=platform.y+1&&p.y>=platform.y)landing=Math.min(landing,platform.y);if(p.y>=landing){p.y=landing;p.vy=0;p.grounded=true;p.jumps=0;}}
  ...
  for(const shot of this.shots){shot.x+=shot.vx*dt;shot.y+=shot.vy*dt;shot.life-=dt;
   ...
   else for(const e of this.room.enemies)if(!e.dead&&!shot.hit.has(e.id)&&Math.abs(shot.x-e.x)<(e.kind==='boss'?44:22)+shot.radius&&Math.abs(shot.y-(e.y-(e.kind==='boss'?52:24)))<(e.kind==='boss'?58:28)+shot.radius){shot.hit.add(e.id);this.hurtEnemy(e,shot.damage,45);shot.pierce--;if(shot.pierce<=0){shot.life=0;break;}}
  }
 ...
 updateEnemy(e:Enemy,dt:number){if(e.dead)return;const p=this.player;e.flash=Math.max(0,e.flash-dt);const dx=p.x-e.x,dist=Math.abs(dx);e.facing=dx>=0?1:-1;e.vx*=Math.exp(-dt*9);
  ...
  if(e.attack>0){e.attack-=dt;if(e.attack<=0){if(boss){const damage=e.hp<e.maxHp*.5?26:21;if(dist<strikeRadius(e.kind)&&p.y>GROUND-85)this.hurtPlayer(damage,'亡王重劈');this.burst(e.x+e.facing*80,GROUND-6,'#e49b73',26);for(const sign of [-1,1])this.shoot(e.x,GROUND-12,sign*210,0,14+(this.floor-1)*2,true,'#e1ab7f',10,1,'亡王地裂');e.timer=e.hp<e.maxHp*.5?1.1:1.75;}else{if(dist<strikeRadius(e.kind)&&Math.abs(e.y-p.y)<75)this.hurtPlayer(elite?18:9,elite?'古堡精英':'骸骨守卫');e.timer=elite?1.1:1.3;}}
  }else{e.timer-=dt;if(dist>range*.7&&dist<1000)e.x+=e.facing*speed*dt;e.x+=e.vx*dt;if(dist<range&&e.timer<=0&&Math.abs(e.y-p.y)<130)e.attack=boss?.85:elite?.6:.45;}
