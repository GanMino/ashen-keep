import type {ClassId,Enemy,Phase,Room,UIState,Upgrade,StoryEvent} from './types';
import {createFloor,floorCount,populate,random} from './world';
import {Sound} from './audio';
import {CHAPTERS,MUTATIONS} from './content';
import {beginJourney,chooseStory,chapterEvent,openRoomEvent} from './story';
import {encounterFor,onEncounterHurt,updateSpecialEnemy,updateHazards,type Hazard} from './encounters';
...
export interface Shot{x:number;y:number;vx:number;vy:number;life:number;damage:number;hostile:boolean;color:string;radius:number;pierce:number;hit:Set<number>;cause:string;bounces?:number}
export interface Pickup{x:number;y:number;value:number;kind:'xp'|'gold'|'heal'}
const UPGRADES:Upgrade[]=[...MUTATIONS,
...
export class Game {
 event:StoryEvent|null=null;memories=0;corruption=0;journal:string[]=[];endingTitle='';endingText='';mutations:Record<string,number>={};shield=0;orbitClock=0;reactionClock=0;hazards:Hazard[]=[];
 ...
 attackDuration=.3;attackCombo=0;attackFacing=1;attackIsSkill=false;comboReset=0;pendingVolley:null|{delay:number;facing:number;skill:boolean}=null;pendingStrike:null|{delay:number;damage:number;range:number;heavy:boolean;facing:number;skill:boolean}=null;effects:Effect[]=[];debris:Debris[]=[];shakeDirection=1;
 ...
 start(seed?:number){
  ...
  this.event=null;this.memories=0;this.corruption=0;this.journal=[];this.endingTitle=this.endingText='';this.mutations={};this.shield=0;this.orbitClock=0;this.reactionClock=0;
  this.loadFloor();this.phase='playing';this.clearInput();this.sound.unlock();this.sound.setMuted(this.sound.muted);this.say('寻找莉娅 · 每层夺取两枚印记，最终击败空心王');
 }
 loadFloor(){this.rooms=createFloor(this.seed,this.floor);this.room=this.rooms[0];this.initRoom();this.player.x=175;this.player.y=440;this.player.vx=this.player.vy=0;this.player.grounded=true;this.cameraX=0;this.clearEffects();this.shield=this.mutations.ward?1:0;this.elites=0;this.bossDefeated=false;}
 clearEffects(){this.pendingVolley=null;this.hazards=[];this.pendingStrike=null;this.effects=[];this.debris=[];this.player.attack=0;this.attackCombo=0;this.comboReset=0;this.particles=[];this.floaters=[];this.shots=[];this.pickups=[];this.hitstop=0;this.shake=0;}
 beginRun(seed?:number){beginJourney(this,seed);}
 chooseEvent(id:string){chooseStory(this,id);}
 shopStock(){const rng=random(this.room.seed+42);const pool=[...MUTATIONS];return [pool.splice(Math.floor(rng()*pool.length),1)[0],pool.splice(Math.floor(rng()*pool.length),1)[0]];}
 offerRelics(mutationsOnly=false){const pool=[...(mutationsOnly?MUTATIONS:UPGRADES)];this.upgrades=[];for(let i=0;i<3;i++)this.upgrades.push(pool.splice(Math.floor(this.rng()*pool.length),1)[0]);this.phase='upgrade';this.clearInput();}
 spawnTrial(){const template={...this.room,kind:'combat' as const};this.room.enemies=populate(template,this.floor).slice(0,12);while(this.room.enemies.length<12){const e={...this.room.enemies[0],id:this.room.id*100+this.room.enemies.length,x:380+this.rng()*(this.room.width-500)};this.room.enemies.push(e);}this.room.cleared=false;}
 ...
 applyUpgrade(id:string){
  ...
  if(MUTATIONS.some(m=>m.id===id)){this.mutations[id]=(this.mutations[id]??0)+1;if(id==='bloodprice'){this.player.maxHp=Math.max(35,this.player.maxHp-20);this.player.hp=Math.min(this.player.hp,this.player.maxHp);this.damage*=1.45;}if(id==='ward'){this.shield=1;if(this.mutations[id]>1)this.lifesteal++;}}
 ...
 hurtEnemy(e:Enemy,damage:number,knock=60,primary=true,novaAllowed=true){if(e.dead)return;
  if(primary){if(this.mutations.ember){e.burn=Math.max(e.burn??0,2.4);e.burnTick=e.burnTick??.5;}if(this.mutations.frost)e.slow=2;if(this.mutations.ember&&this.mutations.frost&&this.time>=this.reactionClock){this.reactionClock=this.time+.35;damage*=1.45;this.effect('fire',e.x,e.y-25,85,'#bbe4e7',.35);this.float(e.x,e.y-95,'蒸汽爆裂','#bbebec');for(const other of this.room.enemies)if(other!==e&&!other.dead&&Math.abs(other.x-e.x)<85)this.hurtEnemy(other,this.damage*.25,0,false);}}
  const armored=e.kind==='elite'&&!e.armorBroken&&['butcher','lancer','duelist','royalguard'].includes(e.variant??'butcher');if(armored)damage*=.78;
  ...
  if(armored&&e.hp<e.maxHp*.55){e.armorBroken=true;e.stagger=.65;this.shatter(e,true);this.effect('armor',e.x,e.y-42,60,'#e8e6c7',.4);this.float(e.x,e.y-110,'破甲','#efba79');this.sound.play('armor');this.hitstop=Math.max(this.hitstop,.065);this.shake=.24;}
  onEncounterHurt(this,e);

  if(e.hp<=0){e.dead=true;this.kills++;this.combo++;this.comboTime=3;this.player.hp=Math.min(this.player.maxHp,this.player.hp+this.lifesteal);this.sound.play('kill');this.shatter(e);this.effect('impact',e.x,e.y-30,30,'#faf0cb',.25);this.burst(e.x,e.y-25,'#c6d6b5',5);if(this.lifesteal)this.effect('soul',e.x,e.y-25,25,'#bd6674',.45,0,1,this.player.x,this.player.y-25);if(this.selectedClass==='ranger')this.effect('impact',e.x,e.y-25,40,'#a5cfd2',.3,3);if((this.selectedClass==='witch'||this.mutations.ember)&&(e.burn??0)>0){this.effect('fire',e.x,e.y-25,100,'#fa9d62',.45);for(const other of this.room.enemies)if(!other.dead&&other.id!==e.id&&Math.abs(other.x-e.x)<145){other.burn=Math.max(other.burn??0,2.4);other.burnTick=.2;}}
   if(novaAllowed&&this.mutations.nova){this.effect('fire',e.x,e.y-25,105,'#edb580',.4);for(const other of this.room.enemies)if(!other.dead&&Math.abs(other.x-e.x)<105)this.hurtEnemy(other,this.damage*.65,100,false,false);}
   ...
   if(e.kind==='boss'){this.bossDefeated=true;this.say(this.floor===this.totalFloors?'空心王已倒下 · 前往右侧炉火，作出最后的选择':'本章领主已陨落 · 前往右侧炉火，继续追寻');this.player.hp=Math.min(this.player.maxHp,this.player.hp+40);this.shake=.4;this.hitstop=.1;}
  }
 }
 hurtPlayer(damage:number,cause:string){const p=this.player;if(p.invuln>0||this.phase!=='playing')return;if(this.shield>0){this.shield--;p.invuln=.7;this.effect('armor',p.x,p.y-30,60,'#dfe7bc',.4);this.sound.play('armor');return;}p.hp=Math.max(0,p.hp-damage);p.invuln=.85;this.shake=.25;this.hitstop=.04;this.combo=0;this.float(p.x,p.y-65,'−'+damage,'#f18982');this.burst(p.x,p.y-28,'#c86461');this.sound.play('hurt');if(p.hp<=0){this.phase='dead';this.deathCause=cause;this.notice='倒在'+cause+'之下';this.saveBest();this.clearInput();}}
 saveBest(){this.best=Math.max(this.best,this.kills);try{localStorage.setItem('ashen-best',String(this.best));}catch{}}
 shoot(x:number,y:number,vx:number,vy:number,damage:number,hostile=false,color='#c2edc9',radius=5,pierce=1,cause='幽魂法术'){if(this.shots.length>=240)return;this.shots.push({x,y,vx,vy,damage,hostile,color,radius,pierce,cause,life:3.2,bounces:!hostile&&this.mutations.ricochet?1:0,hit:new Set()});}
 attack(){const p=this.player;this.attackCombo=this.comboReset>0?this.attackCombo%3+1:1;this.comboReset=.9;this.attackFacing=p.facing;this.attackIsSkill=false;this.attackCount++;
  ...
  }else{this.pendingVolley={delay:.1,facing:p.facing,skill:false};}

  if(this.mutations.echo&&this.attackCount%3===0){this.shoot(p.x,p.y-32,p.facing*570,0,this.damage*.8,false,'#b5cce9',13,6);this.effect('slash',p.x,p.y-30,75,'#b5cce9',.25);}
 ...
 skill(){const p=this.player;this.skillCd=this.skillRate;this.attackCd=Math.max(this.attackCd,.3);this.attackFacing=p.facing;this.attackIsSkill=true;this.attackDuration=.5;p.attack=.5;p.invuln=Math.max(p.invuln,.4);this.shake=.2;
  ...
  else{this.pendingVolley={delay:.22,facing:p.facing,skill:true};}
 ...
 travel(target:number){const old=this.room;for(const item of this.pickups){if(item.kind==='xp')this.xp+=item.value*this.xpMultiplier;else if(item.kind==='gold')this.gold+=item.value;else this.player.hp=Math.min(this.player.maxHp,this.player.hp+item.value);}
  this.room=this.rooms[target];this.initRoom();const door=this.room.doors.find(d=>d.target===old.id);this.player.x=(door?.x??150)+(door&&door.x>this.room.width/2?-65:65);this.player.y=door?.y??440;this.player.vx=this.player.vy=0;this.player.grounded=this.player.y===440;this.player.jumps=0;this.player.invuln=1.2;this.cameraX=Math.max(0,Math.min(this.room.width-W,this.player.x-W*.42));this.clearEffects();this.shield=this.mutations.ward?1:0;this.clearInput();this.say(this.room.kind==='boss'?encounterFor(this.room.enemies[0]).name+' · '+encounterFor(this.room.enemies[0]).tip:this.room.name);this.sound.play('door');this.checkLevel();}
 interactWithWorld(){const p=this.player;
  for(const prop of this.room.props)if(!prop.used&&Math.abs(prop.x-p.x)<75&&Math.abs(prop.y-p.y)<65){
   if(['shop','memory','trial','pact'].includes(prop.kind)){openRoomEvent(this,prop);return;}
   if(prop.kind==='chest'){if(!this.room.cleared){this.say('先消灭房间中的敌人，解除宝箱封印');return;}prop.used=true;this.gold+=15+Math.floor(this.rng()*25);this.offerRelics(true);this.burst(prop.x,prop.y-20,'#e5c77e',28);return;}
   ...
   if(prop.kind==='exit'&&this.bossDefeated){prop.used=true;if(this.floor>=this.totalFloors){prop.used=false;openRoomEvent(this,prop);}else{this.floor++;this.loadFloor();this.player.hp=Math.min(this.player.maxHp,this.player.hp+30);chapterEvent(this);}return;}
  }
  if(!this.room.cleared){this.say('房门被战斗封锁 · 先清除房内敌人');return;}
  ...
  if(this.rooms[door.target].kind==='boss'&&this.elites<2){this.say('领主大门需要两枚精英印记 · '+this.elites+'/2');return;}this.travel(door.target);
 }
 tick(dt:number){
  ...
  const p=this.player;this.comboReset-=dt;this.attackCd-=dt;this.skillCd=Math.max(0,this.skillCd-dt);this.dashCd=Math.max(0,this.dashCd-dt);p.invuln=Math.max(0,p.invuln-dt);p.attack=Math.max(0,p.attack-dt);p.dash=Math.max(0,p.dash-dt);if(this.pendingStrike){this.pendingStrike.delay-=dt;if(this.pendingStrike.delay<=0)this.resolveStrike();}if(this.pendingVolley){this.pendingVolley.delay-=dt;if(this.pendingVolley.delay<=0)this.resolveVolley();}this.comboTime-=dt;if(this.comboTime<=0)this.combo=0;
  ...
  for(const e of this.room.enemies){this.updateEnemy(e,dt);if(this.phase!=='playing')break;}if(this.phase!=='playing')return;updateHazards(this,dt);if(this.phase!=='playing')return;
  if(this.mutations.orbit){this.orbitClock-=dt;if(this.orbitClock<=0){this.orbitClock=.28;for(let i=0;i<Math.min(4,this.mutations.orbit);i++){const a=this.time*3+i*2.4,x=p.x+Math.cos(a)*62,y=p.y-32+Math.sin(a)*35;for(const e of this.room.enemies)if(!e.dead&&Math.abs(e.x-x)<40&&Math.abs(e.y-28-y)<48)this.hurtEnemy(e,this.damage*.35,25,false);}}}
  for(const shot of this.shots){shot.x+=shot.vx*dt;shot.y+=shot.vy*dt;shot.life-=dt;if((shot.bounces??0)>0&&(shot.x<22||shot.x>this.room.width-22)){shot.x=Math.max(22,Math.min(this.room.width-22,shot.x));shot.vx*=-1;shot.bounces!--;}
   if(shot.hostile){if(Math.abs(shot.x-p.x)<shot.radius+14&&Math.abs(shot.y-(p.y-26))<shot.radius+23){this.hurtPlayer(shot.damage,shot.cause);shot.life=0;if(this.phase!=='playing')return;}}
  ...
  if(this.phase==='playing'&&!this.room.cleared&&this.room.enemies.every(e=>e.dead)){this.room.cleared=true;this.gold+=10;this.say(this.room.kind==='boss'?this.notice:'房门开启 · 地图 M 选择下一条路线');if(this.room.kind==='challenge'&&this.room.trialStarted&&!this.room.rewarded){this.room.rewarded=true;this.gold+=30;this.offerRelics(true);}}
 ...
 updateEnemy(e:Enemy,dt:number){if(e.dead)return;const p=this.player;e.flash=Math.max(0,e.flash-dt);e.recovery=Math.max(0,(e.recovery??0)-dt);e.slow=Math.max(0,(e.slow??0)-dt);
  if((e.burn??0)>0){e.burn=Math.max(0,(e.burn??0)-dt);e.burnTick=(e.burnTick??.5)-dt;if(e.burnTick<=0){e.burnTick=.6;this.hurtEnemy(e,this.damage*.22,0,false);this.effect('fire',e.x,e.y-26,23,'#df9c62',.3);}if(e.dead)return;}
...
if(updateSpecialEnemy(this,e,dt))return;const motion=(e.slow??0)>0?.55:1;
const dx=p.x-e.x,dist=Math.abs(dx);e.facing=dx>=0?1:-1;e.vx*=Math.exp(-dt*9);
  if(e.kind==='bat'){if(dist<800){e.x+=Math.sign(dx)*65*dt*motion+e.vx*dt;e.y+=(p.y-20-e.y)*dt*.8;}e.y+=Math.sin(this.time*5+e.id)*dt*9;e.x=Math.max(35,Math.min(this.room.width-35,e.x));if(dist<30&&Math.abs(e.y-p.y)<50)this.hurtPlayer(8+this.floor,'噬月蝠群');return;}
  if(e.kind==='wraith'){if(dist<700){e.x+=(dist>250?Math.sign(dx)*38:-Math.sign(dx)*16)*dt*motion+e.vx*dt;e.y=330+Math.sin(this.time*2+e.id)*18;e.timer-=dt;if(e.timer<=0){const dy=p.y-25-(e.y-28),len=Math.hypot(dx,dy);this.shoot(e.x,e.y-28,dx/len*190,dy/len*190,12+this.floor*2,true,'#d88c99',7);e.timer=2.8;}}e.x=Math.max(35,Math.min(this.room.width-35,e.x));return;}
  if(e.variant==='mushroom'&&dist<240){e.timer-=dt;if(e.timer<=0){this.hazards.push({x:p.x-30,y:370,w:60,h:70,delay:.8,life:.35,damage:10,label:'毒孢子',color:'#b4cb7b',kind:'blast',owner:e.id});e.timer=2.8;}return;}
  ...
  }else if((e.recovery??0)<=0){e.timer-=dt;if(dist>range*.7&&dist<1000)e.x+=e.facing*speed*dt*motion;e.x+=e.vx*dt;if(dist<range&&e.timer<=0&&Math.abs(e.y-p.y)<130)e.attack=boss?.85:elite?.6:.45;}
