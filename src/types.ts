...
export interface Enemy {id:number;kind:'skeleton'|'bat'|'wraith'|'elite'|'boss';x:number;y:number;vx:number;vy:number;hp:number;maxHp:number;facing:number;timer:number;attack:number;flash:number;dead:boolean;homeY:number}
export interface ActorPose {kind:ClassId|Enemy['kind'];x:number;y:number;facing:number;moving:boolean;grounded:boolean;attack:number;flash:number;time:number;elite?:boolean}
